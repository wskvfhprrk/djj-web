    ...
    <div v-loading="loading">
        ...
        <div class="searchBox">
            <el-row v-model="queryParms" :gutter="20" class="operation">
                <el-col class="marginBottom" :span="8">
                    <span class="labelSpan">上级部门ID，一级部门为0： </span>
                    <el-input class="SearchInput" v-model="queryParms.parentId" placeholder="请输入上级部门ID，一级部门为0"></el-input>
                </el-col>
                <el-col class="marginBottom" :span="8">
                    <span class="labelSpan">部门名称： </span>
                    <el-input class="SearchInput" v-model="queryParms.name" placeholder="请输入部门名称"></el-input>
                </el-col>
                <el-col class="marginBottom" :span="8">
                    <span class="labelSpan">排序： </span>
                    <el-input class="SearchInput" v-model="queryParms.orderNum" placeholder="请输入排序"></el-input>
                </el-col>
                <el-col class="marginBottom" :span="8">
                    <span class="labelSpan">是否删除  -1：已删除  0：正常： </span>
                    <el-input class="SearchInput" v-model="queryParms.delFlag" placeholder="请输入是否删除  -1：已删除  0：正常"></el-input>
                </el-col>
                <el-col :span="8">
                ...
                <el-col :span="8">
                    <el-button size="medium" type="primary" v-allow="sysDept_get" @click="search">查询</el-button>
                </el-col>
            </el-row>
        </div>
        <!-- 操作 end-->
        ...
        <div class="resultBox">
            <el-row :gutter="20">
                ...
                <el-button class="addBtn" v-allow="sysDept_post" @click="add" size="medium" type="primary">新增
                </el-button>
            </el-row>
            <el-table :data="list" border style="width: 100%">
                <el-table-column prop="parentId" label="上级部门ID，一级部门为0" align="center"></el-table-column>
                ...
                <el-table-column prop="delFlag" label="是否删除  -1：已删除  0：正常" align="center"></el-table-column>
                <el-table-column label="操作" width="200" align="center">
                    <template slot-scope="scope">
                        <el-button v-allow="sysDept_put" size="mini" type="info" @click="edit(scope.$index,scope.row)">
                            编辑</el-button>
                        <el-button size="mini" type="danger" v-allow="sysDept_*_delete" @click="deleteHandel(scope.$index,scope.row)">
                            删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
...
<script>
    import * as sysDept from '../../api/sysDept.js';
    ...
    export default {
        ...
        methods: {
            ...
            getViewInfo(data) {
                ...
                sysDept.view.r(data.deptId).then(res => {
                    if (res.data.success) {
            ...
            submitHandle() {//新增提交
                ...
                vm.loading = true
                sysDept.add.r(vm.$refs.formPage.form).then(res => {
                    if (res.data.success) {
            ...
            editsubmit() {//新增提交
                ...
                vm.loading = true
                sysDept.edit.r(vm.$refs.formPage.form).then(res => {
                    if (res.data.success) {
            ...
            clock(item, data) {
                ...
                var param = data.deptId;
                sysDept.clock.r(param).then(res => {
                    if (res.data.success) {
            ...
            click(item, data) {
                ...
                var param = data.deptId;
                sysDept.click.r(param).then(res => {
                    if (res.data.success) {
                        vm.$message.success('切换');
                        vm.loading = false;
                        vm.search();
            ...
            deleteHandel(item, data) {
                ...
                var param = data.deptId;
                sysDept.remove.r(param).then(res => {
                    if (res.data.success) {
