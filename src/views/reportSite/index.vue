    ...
    <div v-loading="loading">
        ...
        <div class="searchBox">
            <el-row v-model="queryParms" :gutter="20" class="operation">
                <el-col class="marginBottom" :span="8">
                    <span class="labelSpan">用户编号： </span>
                    <el-input class="SearchInput" v-model="queryParms.userId" placeholder="请输入用户编号"></el-input>
                </el-col>
                <el-col class="marginBottom" :span="8">
                    <span class="labelSpan">上报时间： </span>
                    <el-date-picker class="SearchInput"  v-model="queryParms.reportTime" type="date" placeholder="选择上报时间"></el-date-picker>
                </el-col>
                <el-col class="marginBottom" :span="8">
                ...
                <el-col :span="8">
                    <el-button size="medium" type="primary" v-allow="reportSite_get" @click="search">查询</el-button>
                </el-col>
            </el-row>
        </div>
        <!-- 操作 end-->
        ...
        <div class="resultBox">
            <el-row :gutter="20">
                ...
                <el-button class="addBtn" v-allow="reportSite_post" @click="add" size="medium" type="primary">新增
                </el-button>
            </el-row>
            <el-table :data="list" border style="width: 100%">
                ...
                <el-table-column label="操作" width="200" align="center">
                    <template slot-scope="scope">
                        <el-button v-allow="reportSite_put" size="mini" type="info" @click="edit(scope.$index,scope.row)">
                            编辑</el-button>
                        <el-button size="mini" type="danger" v-allow="reportSite_*_delete" @click="deleteHandel(scope.$index,scope.row)">
                            删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
...
<script>
    import * as reportSite from '../../api/reportSite.js';
    ...
    export default {
        ...
        methods: {
            ...
            fetchData(param) {//查询
                ...
                reportSite.list.r(str).then((res) => {
            ...
            getViewInfo(data) {
                var vm = this;
                //todo 如果没有主键此处为空值，请及时处理
                reportSite.view.r(data.null).then(res => {
                    if (res.data.success) {
            ...
            submitHandle() {//新增提交
                ...
                vm.loading = true
                reportSite.add.r(vm.$refs.formPage.form).then(res => {
                    if (res.data.success) {
            ...
            editsubmit() {//新增提交
                ...
                vm.loading = true
                reportSite.edit.r(vm.$refs.formPage.form).then(res => {
                    if (res.data.success) {
            ...
            clock(item, data) {
                ...
                //todo 如果没有主键此处为空值，请及时处理
                var param = data.null;
                reportSite.clock.r(param).then(res => {
                    if (res.data.success) {
            ...
            click(item, data) {
                ...
                //todo 如果没有主键此处为空值，请及时处理
                var param = data.null;
                reportSite.click.r(param).then(res => {
                    if (res.data.success) {
                        vm.$message.success('切换');
                        vm.loading = false;
                        vm.search();
            ...
            deleteHandel(item, data) {
                ...
                //todo 如果没有主键此处为空值，请及时处理
                var param = data.null;
                reportSite.remove.r(param).then(res => {
                    if (res.data.success) {
