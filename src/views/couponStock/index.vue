    ...
    <div v-loading="loading">
        ...
        <div class="searchBox">
            <el-row v-model="queryParms" :gutter="20" class="operation">
                <el-col class="marginBottom" :span="8">
                    ...
                    <el-input class="SearchInput" v-model="queryParms.couponId" placeholder="请输入编号"></el-input>
                </el-col>
                <el-col class="marginBottom" :span="8">
                    <span class="labelSpan">店铺（最后在哪个店铺发出）： </span>
                    <el-input class="SearchInput" v-model="queryParms.shopId" placeholder="请输入店铺（最后在哪个店铺发出）"></el-input>
                </el-col>
                <el-col class="marginBottom" :span="8">
                    <span class="labelSpan">状态（0-库存 1-已领取 2-已使用 3-过期 4-作废）： </span>
                    <el-input class="SearchInput" v-model="queryParms.status" placeholder="请输入状态（0-库存 1-已领取 2-已使用 3-过期 4-作废）"></el-input>
                </el-col>
                <el-col class="marginBottom" :span="8">
                    <span class="labelSpan">会员ID（会员领取后绑定）： </span>
                    <el-input class="SearchInput" v-model="queryParms.memberId" placeholder="请输入会员ID（会员领取后绑定）"></el-input>
                </el-col>
                <el-col class="marginBottom" :span="8">
                ...
                <el-col :span="8">
                    <el-button size="medium" type="primary" v-allow="couponStock_get" @click="search">查询</el-button>
                </el-col>
            </el-row>
        </div>
        <!-- 操作 end-->
        ...
        <div class="resultBox">
            <el-row :gutter="20">
                ...
                <el-button class="addBtn" v-allow="couponStock_post" @click="add" size="medium" type="primary">新增
                </el-button>
            </el-row>
            <el-table :data="list" border style="width: 100%">
                <el-table-column prop="couponId" label="编号" align="center"></el-table-column>
                <el-table-column prop="shopId" label="店铺（最后在哪个店铺发出）" align="center"></el-table-column>
                <el-table-column prop="status" label="状态（0-库存 1-已领取 2-已使用 3-过期 4-作废）" align="center"></el-table-column>
                <el-table-column prop="memberId" label="会员ID（会员领取后绑定）" align="center"></el-table-column>
                <el-table-column prop="createTime" label="创建时间" align="center"></el-table-column>
                <el-table-column prop="updateTime" label="修改时间" align="center"></el-table-column>
                <el-table-column label="操作" width="200" align="center">
                    <template slot-scope="scope">
                        <el-button v-allow="couponStock_put" size="mini" type="info" @click="edit(scope.$index,scope.row)">
                            编辑</el-button>
                        <el-button size="mini" type="danger" v-allow="couponStock_*_delete" @click="deleteHandel(scope.$index,scope.row)">
                            删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
...
<script>
    import * as couponStock from '../../api/couponStock.js';
    ...
    export default {
        ...
        methods: {
            ...
            fetchData(param) {//查询
                ...
                couponStock.list.r(str).then((res) => {
            ...
            getViewInfo(data) {
                ...
                couponStock.view.r(data.couponNo).then(res => {
                    if (res.data.success) {
            ...
            submitHandle() {//新增提交
                ...
                vm.loading = true
                couponStock.add.r(vm.$refs.formPage.form).then(res => {
                    if (res.data.success) {
            ...
            editsubmit() {//新增提交
                ...
                vm.loading = true
                couponStock.edit.r(vm.$refs.formPage.form).then(res => {
                    if (res.data.success) {
            ...
            clock(item, data) {
                ...
                var param = data.couponNo;
                couponStock.clock.r(param).then(res => {
                    if (res.data.success) {
            ...
            click(item, data) {
                ...
                var param = data.couponNo;
                couponStock.click.r(param).then(res => {
                    if (res.data.success) {
                        vm.$message.success('切换');
                        vm.loading = false;
                        vm.search();
            ...
            deleteHandel(item, data) {
                ...
                var param = data.couponNo;
                couponStock.remove.r(param).then(res => {
                    if (res.data.success) {
